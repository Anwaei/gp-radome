...
delta_t_disc=0.001;
% out = fu_function_interp(out, delta_t_disc);
...
for i=1:length(x1_disc)
    if i==1
        ...
        x3_disc(i)=x_curr(3);
        u_disc(i)=u_curr;
        rho_theta_disc(i) = fu_function_rho_theta(x_curr,u_curr);
    end
end

% (3) testing, plot figure (CONTinuous VS DISCrete)
x1_disc = zeros(size(x1_disc)); x2_disc = zeros(size(x2_disc)); x3_disc = zeros(size(x3_disc));
...
[theta_s_sort, theta_s_sort_index] = sort(theta_s);
rho_theta_sort = rho_theta(theta_s_sort_index);
lambda_r_sort = lambda_r(theta_s_sort_index);
figure
plot(rad2deg(theta_s_sort), rho_theta_sort);
xlabel('\theta_s (deg)')
ylabel('\rho')
title('radome slope vs look angle');
figure
plot(rad2deg(theta_s_sort), rad2deg(lambda_r_sort));
title('radome aberration vs look angle');
xlabel('\theta_s (deg)')
ylabel('\lambda_r (deg)')
% figure
% index_10 = (rad2deg(theta_s_sort)>-10)&(rad2deg(theta_s_sort)<10);
% plot(rad2deg(theta_s_sort(index_10)), rad2deg(lambda_r_sort(index_10)));
diff_lambda_r_theta_s = diff(lambda_r_sort)./diff(theta_s_sort);
figure
plot(rad2deg(theta_s_sort(1:length(diff_lambda_r_theta_s))),diff_lambda_r_theta_s);
xlabel('\theta_s (deg)')
ylabel('\rho')
title('radome slope vs look angle calculated by diff');


% examine rho continuous and discrete
rho_theta_test = zeros(length(t),1);
lambda_s_test = zeros(length(t),1);
lambda_test = zeros(length(t),1);
theta_M_test = zeros(length(t),1);
alpha_M_test = zeros(length(t),1);
gamma_M_test = zeros(length(t),1);
for i = 1:length(t)
    x = [x1_cont(i), x2_cont(i), x3_cont(i)];
    u = u_cont(i);
    rho_theta_test(i) = fu_function_rho_theta(x,u);
    
    lambda_t = x(1);
    gamma_M_t = x(3);
    gamma_dot_t = function_radome_3(u);
    T_alpha_t = 1;
    alpha_M_t = T_alpha_t*gamma_dot_t;
    theta_M_t = alpha_M_t + gamma_M_t;
    gamma_M_test(i) = gamma_M_t;
    alpha_M_test(i) = alpha_M_t;
    lambda_test(i) = lambda_t;
    theta_M_test(i) = theta_M_t;
    lambda_s_test(i) = lambda_t - theta_M_t;
end

figure
plot(t,rho_theta,t,rho_theta_test);
legend('cont','disc')
title('\rho')
figure
plot(t,rad2deg(theta_s),t,rad2deg(lambda_s_test));
legend('cont','disc')
title('\theta_s')
% figure
% plot(t,rad2deg(lambda),t,rad2deg(lambda_test));
% legend('cont','disc')
% title('\lambda')
figure
plot(t,rad2deg(theta_M),t,rad2deg(theta_M_test));
legend('cont','disc')
title('\theta_M')
% figure
% plot(t,rad2deg(alpha_M),t,rad2deg(alpha_M_test));
% legend('cont','disc')
% title('\alpha_M')
% figure
% plot(t,rad2deg(gamma_M),t,rad2deg(gamma_M_test));
% legend('cont','disc')
% title('\gamma_M')
figure
plot(t,rho_theta,0:delta_t_disc:t(end),rho_theta_disc);
legend('\rho_\theta cont','\rho_\theta disc')
...
subplot(3,3,3)
index_m=2/delta_t_cont:100:length(theta_s);
index_s=2/delta_t_cont:75:length(theta_s);
